...
| D-028 | Product XLSX previews are short-lived in process and stock is ledger-protected | The file is parsed before an explicit one-time confirmation and is never permanently retained; existing stock is read-only, while new opening stock creates an audited `opening_balance` movement in the atomic import transaction. |
