...
- Release 3.0.1 desktop launcher uses port `8002`, `runtime/`, and fixed server
...
On 2026-07-29, `python -m unittest discover -s tests -q` completed 153 tests
successfully with one filesystem-capability skip, including private-LAN staff
login/session access, outside-LAN rejection, customer contact UI, delivery payment-method confirmation,
...
Version 3.0.1 is the current customer-delivery baseline. It retains Version 3.0.0
hardening and permits staff POS use from `192.168.0.0/24` while requests outside
that LAN remain blocked. The Windows server address is static
