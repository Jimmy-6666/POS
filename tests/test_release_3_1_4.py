...
class Release314Tests(unittest.TestCase):
    ...
    def test_release_identity_is_314_and_uat_is_explicit(self):
        self.assertEqual(DEFAULT_APP_VERSION, "3.1.4")
        ...
        self.assertIn("R3.1.4", base)
        self.assertIn("filename='js/pos.js',v=38", pos)
        self.assertIn('set "POS_PORT=8001"', start_uat)
        self.assertIn('set "POS_APP_VERSION=3.1.4"', start_uat)
        self.assertIn("3.1.4 UAT", start_uat)
        self.assertIn("POS Desktop Launcher · Version 3.1.4", launcher)
        self.assertIn('messagebox.showerror("POS 3.1.4"', launcher)
