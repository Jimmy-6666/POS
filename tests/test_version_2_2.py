...
class Version310ReleaseTests(unittest.TestCase):
    def test_release_identity_is_consistent(self):
        self.assertEqual(DEFAULT_APP_VERSION, "3.1.1")
        self.assertIn("R3.1.1", (ROOT / "pos_app" / "templates" / "base.html").read_text(encoding="utf-8"))
        launcher = (ROOT / "pos_desktop.py").read_text(encoding="utf-8")
        self.assertIn("Saengngam POS 3.1.1", launcher)
        ast.parse(launcher)
        production = (ROOT / "production-common.ps1").read_text(encoding="utf-8")
        self.assertIn('AppVersion = "3.1.1"', production)
