...
class Version24ReleaseTests(unittest.TestCase):
    def test_release_identity_is_consistent(self):
        self.assertEqual(DEFAULT_APP_VERSION, "2.4")
        self.assertIn("R2.4", (ROOT / "pos_app" / "templates" / "base.html").read_text(encoding="utf-8"))
        launcher = (ROOT / "pos_desktop.py").read_text(encoding="utf-8")
        self.assertIn("Saengngam POS 2.4", launcher)
        ast.parse(launcher)
        self.assertTrue((ROOT / "VERSION_2.4.md").is_file())
