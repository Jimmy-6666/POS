...
class Version306ReleaseTests(unittest.TestCase):
    def test_release_identity_is_consistent(self):
        self.assertEqual(DEFAULT_APP_VERSION, "3.0.6")
        self.assertIn("R3.0.6", (ROOT / "pos_app" / "templates" / "base.html").read_text(encoding="utf-8"))
        launcher = (ROOT / "pos_desktop.py").read_text(encoding="utf-8")
        self.assertIn("Saengngam POS 3.0.6", launcher)
        ast.parse(launcher)
        production = (ROOT / "production-common.ps1").read_text(encoding="utf-8")
        self.assertIn('AppVersion = "3.0.6"', production)
        self.assertIn("$env:POS_APP_VERSION = $Context.AppVersion", production)
        self.assertIn('ServerIp = "192.168.0.200"', production)
        self.assertTrue((ROOT / "VERSION_3.0.6.md").is_file())
