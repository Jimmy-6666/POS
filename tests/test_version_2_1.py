...
class Version21DisplayTests(unittest.TestCase):
    ...
    def test_launcher_limits_kiosk_printing_to_receipt_agent(self):
        ...
        self.assertIn('os.environ.get("POS_RECEIPT_PRINTER", "POSPrinter POS-80")', source)
        self.assertIn("def configure_attach_only_default_printer", source)
        self.assertIn("if not ATTACH_ONLY or not RECEIPT_PRINTER_NAME", source)
        self.assertIn("default_printer_configured", source)
        staff_login(self.client)
