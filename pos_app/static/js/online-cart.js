(() => {
  ...
  function add(product,button){
    const cart=read(),item=cart.find(row=>row.product_uuid===product.product_uuid),step=product.allow_decimal_quantity?0.001:1,next=(item?.quantity||0)+step,max=Math.min(product.available_quantity,product.online_max_quantity||Infinity);
    if(next>max)return;
