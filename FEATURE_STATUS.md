...
| Release 3.1.8 80 mm POSPrinter driver-form and GDI-logo correction | Production deployed 2026-08-03; no migration, API, sale, or stock change; native `80(72)mm x 297mm` form preserves the 72 mm printable area on the 80 mm roll and the approved logo is drawn in the SYSTEM GDI job; focused 3/3, no-print GDI compile, full UAT 205/205 (1 capability skip), and customer-confirmed physical reprint | `VERSION_3.1.8.md`, `pos_app/services/print_jobs.py`, `tests/test_receipt_format.py` |
| LINE group product-maintenance bot | Production package `v3.1.5-prd-linebot-hotfix1` includes signed price/create integration, persistent SYSTEM startup configuration, post-reboot attach-only shortcut ACL repair, exact-satang cost input, one Manager-group allowlist, one-minute timeout notice, indefinite five-minute retry, 24-hour source-image-reference retention, linear barcode only, and no product-image maintenance; final focused 33/33 and full 221 passed/1 skip | `docs/LINE_BOT_SETUP.md`, `tests/test_line_bot_integration.py`, `tests/test_line_bot_workflow.py`, `tests/test_runtime_paths.py` |
| Release 3.1.6 LINE Bot Live Production | VPS-only promotion of Vision and shared legacy Quote flows; any allowed-group member may progress and confirm; no Windows POS deployment | `VERSION_3.1.6.md`, `tests/test_line_bot_workflow.py`, `tests/test_line_bot_vision_workflow.py` |
| Release 3.1.7 LINE Bot Gemini budget control | Live VPS uses `gemini-3.1-flash-lite`, explicit pricing, one attempt, barcode/POS lookup first, and a Thailand-calendar USD 1 application cap with manual fallback | `VERSION_3.1.7.md`, `tests/test_line_bot_gemini_budget.py`, `tests/test_line_bot_vision_workflow.py` |
| Release 3.1.9 LINE Bot reply/quota hotfix | Live VPS sends the normal final POS result by Reply; delayed results use a durable outbox and stable retry key; focused 48/48 and full 252 with 1 existing skip | `VERSION_3.1.9.md`, `tests/test_line_bot_workflow.py` |
| Release 3.2.0 cumulative source | Verified source combines POS `v3.1.8` and LINE Bot `v3.1.9` on one line with 3.2.0 identity and resolved documentation history; receipt and Bot tree comparisons passed, focused 73/73, full 257 passed with 1 existing skip; live deployment is unchanged by source publication | `VERSION_3.2.0.md`, `tests/test_release_3_2_0.py` |
| Release 3.2.1 LINE Bot manual commands | Deployed to the Live VPS 2026-08-08. Adds `/check barcode` as a one-shot signed lookup and `/bot barcode|name|price` as a group-shared confirmation flow for unknown/Placeholder products. Named products are not overwritten; neither command calls Gemini. Focused 60/60; full 266 passed with 1 existing skip; VPS release tests 7/7 and post-switch health/SQLite/log checks passed. VPS LINE Bot only; `pos_app` remains 3.2.0 with no migration. | `VERSION_3.2.1.md`, `tests/test_line_bot_workflow.py`, `tests/test_release_3_2_1.py` |
